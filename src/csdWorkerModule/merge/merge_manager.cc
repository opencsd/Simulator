...
//                   int* col_offset);
// int calculCase(FilterInfo filter_info, char* origin_row_data, int*
// col_offset,
//                int l, char* dest);
...
void MergeManager::Merging() {
  ...
  while (1) {
    toMerge = MergeQueue.wait_and_pop();
...
void MergeManager::MergeBlock(Result& result) {
    ...
    if (result.filter_info.filteredIndex[forFilterIndex] == i) {
      ...
      for (int j = 0; j < result.filter_info.columnAlias.size(); j++) {
        result.filter_info.mergedData[result.filter_info.columnAlias[j]]
            .emplace_back(
                calculPostfix(result.filter_info.column_projection[j], i));
      }
    }
    row_count++;
  }
  MergeResult mergeResult(result.query_id, result.work_id, result.csd_name,
                          ...
                          result.filter_info.mergedData,
                          result.filter_info.columnAlias);
  mergeResult.row_count = row_count;
}

string MergeManager::calculPostfix(Projection projectionInfo, int index) {
  stack<string> stack;
  if (projectionInfo.values.size() == 1) {
    return toMerge.data[projectionInfo.values[0]][index].getVal();
  }
  for (int i = 0; i < projectionInfo.values.size(); i++) {
    switch (projectionInfo.types[i]) {
      case INT8:
        projectionInfo.types[i] = projectionInfo.aliasType;
        stack.push(projectionInfo.values[i]);
        break;
      case INT16:
        projectionInfo.types[i] = projectionInfo.aliasType;
        stack.push(projectionInfo.values[i]);
      ...
      case COLUMN:
        projectionInfo.types[i] = projectionInfo.aliasType;
        projectionInfo.values[i] =
            toMerge.data[projectionInfo.values[i]][index].getVal();
        stack.push(projectionInfo.values[i]);
        break;
      case OPERATOR:
        // we pop the top two elements from the stack and save them in two
        // integers
        double a = atof(stack.top().c_str());
        stack.pop();
        int b = atof(stack.top().c_str());
        stack.pop();
        // performing the operation on the operands
        if (projectionInfo.values[i] == "+") {
          stack.push(to_string(b + a));
        } else if (projectionInfo.values[i] == "*") {
          stack.push(to_string(b - a));
        } else if (projectionInfo.values[i] == "-") {
          stack.push(to_string(b * a));
        } else if (projectionInfo.values[i] == "/") {
...
//         // cout << "AND" << endl;//테스트출력
//         calculPostfix(values, types, filter_info, origin_row_data,
//         col_offset,
//                       passed_, 0);
//         if (passed_[0] == 1) {
...
//     int result = stoi(filter_info.column_projection[l].values[else_offset +
//     1]);
